...
export async function listProjects(): Promise<Project[]> {
  const rows = (await (
    await getDb()
  ).select<SqlRow[]>("SELECT * FROM projects ORDER BY updated_at DESC, name ASC")) as SqlRow[];
  return rows.map(rowToProject);
}

export async function createProject(name: string, description: string): Promise<Project> {
  const db = await getDb();
  const result = await db.execute("INSERT INTO projects (name, description) VALUES ($1, $2)", [
    name,
    description,
  ]);
  const id = Number(result.lastInsertId);
  const rows = (await db.select<SqlRow[]>("SELECT * FROM projects WHERE id = $1", [
    id,
  ])) as SqlRow[];
...
export async function listGpxFiles(): Promise<GpxFile[]> {
  const rows = (await (
    await getDb()
  ).select<SqlRow[]>("SELECT * FROM gpx_files ORDER BY imported_at DESC, id DESC")) as SqlRow[];
...
export async function getGpxFile(id: number): Promise<GpxFile | null> {
  const rows = (await (
    await getDb()
  ).select<SqlRow[]>("SELECT * FROM gpx_files WHERE id = $1", [id])) as SqlRow[];
...
export async function getSetting(key: string): Promise<string | null> {
  const rows = (await (
    await getDb()
  ).select<SqlRow[]>("SELECT value FROM settings WHERE key = $1", [key])) as SqlRow[];
...
function rowToGpxFile(row: SqlRow): GpxFile {
  return {
    ...
    boundsMinLat:
      row.bounds_min_lat === null || row.bounds_min_lat === undefined
        ? null
        : Number(row.bounds_min_lat),
    boundsMinLon:
      row.bounds_min_lon === null || row.bounds_min_lon === undefined
        ? null
        : Number(row.bounds_min_lon),
    boundsMaxLat:
      row.bounds_max_lat === null || row.bounds_max_lat === undefined
        ? null
        : Number(row.bounds_max_lat),
    boundsMaxLon:
      row.bounds_max_lon === null || row.bounds_max_lon === undefined
        ? null
        : Number(row.bounds_max_lon),
  };
}
